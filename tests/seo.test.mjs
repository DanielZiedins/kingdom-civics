...
test("start path scripture index and new civic content ship", async () => {
  ...
  assert.match(learn, /what-to-bring-to-the-poll/);
  assert.match(learn, /discern-local-candidates/);
  assert.match(issues, /truth-public-speech/);
  assert.match(issues, /disability-access/);
  ...
  assert.match(election, /communityPolls/);
  assert.match(election, /communityPollCount/);
  assert.match(election, /findWard/);
  assert.match(election, /getWeekendStatus/);
  ...
  assert.match(blog, /before-hamilton-community-polls/);
